...
async function createApp(edgespark) {
  ...
  app.get("/api/products", async (c) => {
    const products2 = await edgespark.db.select().from(db_schema_exports.products).orderBy(desc(db_schema_exports.products.createdAt));
    return c.json({ data: products2 });
  });
  app.get("/api/products/:id", async (c) => {
    const id = Number(c.req.param("id"));
    const product = await edgespark.db.select().from(db_schema_exports.products).where(eq(db_schema_exports.products.id, id)).get();
    if (!product) return c.json({ error: "Product not found" }, 404);
    return c.json({ data: product });
  });
  app.post("/api/products", async (c) => {
    const body = await c.req.json();
    const result = await edgespark.db.insert(db_schema_exports.products).values({
      name: body.name,
  ...
  app.put("/api/products/:id", async (c) => {
    const id = Number(c.req.param("id"));
    const body = await c.req.json();
    const result = await edgespark.db.update(db_schema_exports.products).set({
      name: body.name,
      category: body.category,
      price: Number(body.price),
      stock: Number(body.stock),
      imageUrl: body.imageUrl,
      description: body.description
    }).where(eq(db_schema_exports.products.id, id)).returning();
    return c.json({ data: result[0] });
  });
  app.delete("/api/products/:id", async (c) => {
    const id = Number(c.req.param("id"));
    await edgespark.db.delete(db_schema_exports.products).where(eq(db_schema_exports.products.id, id));
    return c.json({ success: true });
  });
  app.get("/api/orders", async (c) => {
    const orders2 = await edgespark.db.select().from(db_schema_exports.orders).orderBy(desc(db_schema_exports.orders.createdAt));
  ...
  app.post("/api/orders", async (c) => {
    ...
    for (const item of body.items) {
      await edgespark.db.insert(db_schema_exports.orderItems).values({
        orderId,
